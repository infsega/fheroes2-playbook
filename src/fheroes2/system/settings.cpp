...
#ifdef __PLAYBOOK__
    opt_global.SetModes(POCKETPC_HIDE_CURSOR);
    opt_global.SetModes(POCKETPC_TAP_MODE);
    opt_global.SetModes(POCKETPC_DRAG_DROP_SCROLL);
#endif
...
#ifdef __PLAYBOOK__
    dirs.push_back("shared/misc/fheroes2");
    dirs.push_back("app/native/assets");
    dirs.push_back("data/files");
#else
    // from build
...
{
#ifdef __PLAYBOOK__
  std::string rv = std::string("data") + SEPARATOR + "files" + SEPARATOR + subdir;
  MKDIR("data/files");
  MKDIR(rv.c_str());
	return rv;
#else
