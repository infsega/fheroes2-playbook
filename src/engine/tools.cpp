...
std::string String::Trim(const std::string& i_str)
{
...
std::string String::Lower(const std::string& str)
{
	...
	for ( std::string::size_type pos = 0; pos < s.length(); ++pos )
		s[pos] = ::tolower(s[pos]);
    //std::transform(str.begin(), s.end(), s.begin(), ::tolower);
    return s;
}
...
{
	std::string s = str;
    std::transform(s.begin(), s.end(), s.begin(), std::bind2nd(std::ptr_fun(&std::toupper<char>), std::locale("")));
    return s;
}
